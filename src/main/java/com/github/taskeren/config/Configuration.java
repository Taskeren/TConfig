...
import static com.github.taskeren.config.Property.Type.*;
...
public class Configuration {
	...
	private void runConfiguration(File file, String configVersion) {
		...
		String path = file.getAbsolutePath().replace(File.separatorChar, '/').replace("/./", "/");
		if(PARENT != null) {
			...
			try {
				...
				LOG.error("An exception occurred while loading config file {}. This file will be renamed to {} and a new config file will be generated.", file.getName(), fileBak.getName(), e);
	 ...
	 * @param defaultValues     an array containing the default values
	 * @param comment           a String comment
	 * @param isListLengthFixed boolean for whether this array is required to be a
	 *                          specific length (defined by the default value array
	 *                          length or maxListLength)
	 * @param maxListLength     the maximum length of this array, use -1 for no max
	 *                          length
	 ...
	 * @param defaultValue the default value
	 * @param comment      a String comment
	 * @param validValues  an array of valid values that this Property can be set
	 *                     to. If an array is provided the Config GUI control will
	 *                     be a value cycle button.
	 ...
	 * @param defaultValue       the default value
	 * @param comment            a String comment
	 * @param validValues        an array of valid values that this Property can be
	 *                           set to. If an array is provided the Config GUI
	 *                           control will be a value cycle button.
	 * @param validValuesDisplay an array of the config GUI display versions of the
	 *                           valid values that this Property can be set to.
	 * @return a string Property with the defined validValues array,
	 * validationPattern = null
	 */
	public Property get(String category, String key, String defaultValue, String comment, String[] validValues,
...
//			FMLLog.log.error("Error while saving config {}.", fileName, e);
	...
	public void copyCategoryProps(Configuration fromConfig, String[] ctgys) {
		...
		for(String ctgy : ctgys)
			if(fromConfig.hasCategory(ctgy) && this.hasCategory(ctgy)) {
				ConfigCategory thisConfCate = this.getCategory(ctgy);
				ConfigCategory fromConfCate = fromConfig.getCategory(ctgy);
				for(Entry<String, Property> entry : thisConfCate.getValues().entrySet())
					if(fromConfCate.containsKey(entry.getKey()))
						thisConfCate.put(entry.getKey(), fromConfCate.get(entry.getKey()));
			}
	}
	...
	public String getString(String name, String category, String defaultValue, String comment) {
		return getString(name, category, defaultValue, comment, (Pattern) null);
	}
	...
	public String getString(String name, String category, String defaultValue, String comment, String[] validValues) {
		return getString(name, category, defaultValue, comment, validValues, null);
	}
	 ...
	 * @param defaultValue       Default value of the property.
	 * @param comment            A brief description what the property does.
	 * @param validValues        A list of valid values that this property can be
	 *                           set to.
	 * @param validValuesDisplay an array of the config GUI display versions of the
	 *                           valid values that this Property can be set to.
	 * @return The value of the new string property.
	...
	public String getString(String name, String category, String defaultValue, String comment, String[] validValues,
							String[] validValuesDisplay) {
		return setPropertyAndGetString(name, category, defaultValue, comment, validValues, validValuesDisplay);
	}
	...
	public String[] getStringList(String name, String category, String[] defaultValues, String comment) {
		return getStringList(name, category, defaultValues, comment, null);
	}
	...
	public String[] getStringList(String name, String category, String[] defaultValue, String comment,
								  String[] validValues, String[] validValuesDisplay) {
		return setPropertyAndGetStringList(name, category, defaultValue, comment, validValues, validValuesDisplay);
	}
	...
	public String[] getStringList(String name, String category, String[] defaultValue, String comment,
								  String[] validValues) {
		return setPropertyAndGetStringList(name, category, defaultValue, comment, validValues, null);
	}
	...
	public float getFloat(String name, String category, float defaultValue, float minValue, float maxValue,
		...
		try {
			...
			LOG.error("Failed to get float for {}/{}", name, category, e);
		}
		return defaultValue;
	}
	...
	public double getDouble(String name, String category, double defaultValue, double minValue, double maxValue,
		...
		try {
			...
			LOG.error("Cannot parse " + prop.getString() + " to Double.", e);
		}
		return defaultValue;
	}
